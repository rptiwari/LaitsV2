    ...
    try {
      ...
      if (task.getPhaseTask()!=Task.THE_BREAK) {
        ...
        for (Element e : Vertex) {
          ...
          if (((Node.getType()==laits.graph.Vertex.STOCK) || (Node.getType()==laits.graph.Vertex.FLOW)) && (!InputNodesList.isEmpty())) {
            String edgeType;
            String inputName;
          ...
          if (((Node.getType()==laits.graph.Vertex.STOCK) || (Node.getType()==laits.graph.Vertex.FLOW)) && (!OutputNodesList.isEmpty())) {
        ...
        if (task.getTypeTask()!=laits.data.Task.DEBUG) {
          //ANDREW TO CHECK    
          ...
          for (Element e : DebugVertex) {
            ...
            if (((debugNode.getType()==laits.graph.Vertex.STOCK) || (debugNode.getType()==laits.graph.Vertex.FLOW)) && (!InputNodesList.isEmpty())) {
              String edgeType;
              String inputName;
              Vertex inputVertex = null;
            ...
            //this line contains the node's input. It is only useful for function node.
            if (((debugNode.getType()==laits.graph.Vertex.STOCK) || (debugNode.getType()==laits.graph.Vertex.FLOW)) && (!OutputNodesList.isEmpty())) {
            ...
            if (debugNode.getType()==laits.graph.Vertex.STOCK) {
              debugNode.addInitialFormula(e.element("Equation").getText());
            } 
            else if (debugNode.getType()==laits.graph.Vertex.CONSTANT) 
            {
              debugNode.setCalculationsButtonStatus(debugNode.NOSTATUS);
            }

            debugNode.correctValues.clear();
            if (debugNode.getType()==laits.graph.Vertex.CONSTANT) 
            {
              debugNode.setInitialValue(Double.parseDouble(e.element("InitialValue").getText()));
            } 
            else if (debugNode.getType()==laits.graph.Vertex.STOCK) 
            {
              debugNode.setInitialValue(Double.parseDouble(e.element("InitialValue").getText()));
              debugNode.addInitialFormula(e.element("Equation").getText());
            } 
            else if (debugNode.getType()==laits.graph.Vertex.FLOW) 
            {
              debugNode.addInitialFormula(e.element("Equation").getText());
            }
        ...
        if (task.listOfVertexesDebug != null) {
          ...
          for (int n = 0; n < task.listOfVertexesDebug.size(); n++) {
            ...
            for (int m = 0; m < task.listOfVertexes.size(); m++) {
              ...
              if (debugVertex.getNodeName().equals(correctVertex.getNodeName())) 
              {
                if (debugVertex.getType()==laits.graph.Vertex.CONSTANT) 
                {
                ...
                else if (debugVertex.getType()==laits.graph.Vertex.FLOW) 
                {
                  if (correctVertex.getType()==laits.graph.Vertex.FLOW) 
                  {
                ...
                else if (debugVertex.getType()==laits.graph.Vertex.STOCK) 
                {
                  if (correctVertex.getType()==laits.graph.Vertex.STOCK) 
                  {
                    if (correctVertex.getCorrectInputs().equals(debugVertex.getCorrectInputs()) && correctVertex.getCorrectOutputs().equals(debugVertex.getCorrectOutputs()))
                      debugVertex.setInputsButtonStatus(debugVertex.CORRECT);
                    else
