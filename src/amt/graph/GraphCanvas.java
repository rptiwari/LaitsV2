...
public class GraphCanvas extends JPanel implements FocusListener, ActionListener, KeyListener, MouseListener, MouseMotionListener, ComponentListener, Scrollable {
  ...
  private JButton runButton, shortDescriptionButton;
  ...
  public GraphCanvas(Main jf) {
    ...
    try {
      ...
      logs.concatOut(Logger.DEBUG, "GraphCanvas.GraphCanvas.1", de.toString());    
    }
  ...
  private void initRunButton() {
    ...
    runButton.addActionListener(new java.awt.event.ActionListener() {
      ...
      private void runButtonActionPerformed(ActionEvent evt, GraphCanvas gc) {
        ...
        if (gc.runButton.getForeground() != Color.GRAY) {
    } else {
  ...
  private void initFirstProblem() {
    ...
    try
    {
        String user = Database.getInstance().getUser();
        if (Main.MetaTutorIsOn) {
            Main.segment_DepthDetector.start_first_detector(user, DepthDetector.MT);
        }
        else
        {
            Main.segment_DepthDetector.start_first_detector(user, DepthDetector.CONTROL);
        }
    }
    catch (CommException e)
    {
      logger.out(Logger.DEBUG, "Main.getDatabase.1", e.getMessage());
    }
  ...
  public final void paintComponent(Graphics g) {
    ...
    if (passed == true) {
      ...
      if (!studentReceivedLevelPoint && currentTask.getLevel() > 0 && practice == false) {
  ...
  public void loadLevel(int taskNum) {
    ...
    currentTask.ClearTaskFromActual();
    ...
    if (task != null) {
      ...
      listOfVertexes = currentTask.getVertexNames();
...
///Deleted amd moved to MenuBar
      ...
      if (currentTask.getTitle() == null) {
        ...
        this.setFont(this.normal);
        frame.setTitle("Affective Meta Tutor - " + currentTask.getTitle());
      }
    }
    problemIndex = 0;
    run.setForeground(Color.GRAY);
  ...
  public boolean checkNewNodeButton(){
    if (InstructionPanel.canNewNodeButtonBePressed || server.getActualTask().getPhaseTask() != Task.INTRO || (!InstructionPanel.stopIntroductionActive && !InstructionPanel.goBackwardsSlides)) {
        if (task.listOfVertexesDebug != null) {
          System.out.println("Number of vertexes given as debug:" + task.listOfVertexesDebug.size());
...
/*    if (ee != null) {
 ...
 //   logger.concatOut(Logger.ACTIVITY, "MenuBar.doneButtonActionPerformed.1", Integer.toString(id)); 
  ...
  public boolean canRun() {
    for(int i=0;i<graph.getVertexes().size();i++)
      if(!graph.getVertexes().get(i).getHasBlueBorder())
        return false;
    return true;
  }
    ...
    if (graph.getSelected() instanceof Edge) {
      ...
      if ((v != null) && (v != ed.start)) {
        ...
        if (validate(ed)) {
          ...
          logs.concatOut(Logger.ACTIVITY, "GraphCanvas.mouseReleased.2", ed.start.getNodeName() + "-" + ed.end.getNodeName());
        } else {
          // HELEN
          menuVertex.isSelectedOnCanvas = false;
...
//    private boolean existEdgeBetween(Vertex allEdges, Vertex counter){
   ...
   * @return
   */
  public boolean checkNodeForListInputs(int vertexIndex) {
//    return NodeEditor.getInstance((Vertex) graph.getVertexes().get(vertexIndex), graph, this, false, true).getInputsPanel().checkForListInputs();
    return false;
  }
   ...
   * @return
   */
  public boolean checkNodeForListInputsyntactics(int vertexIndex) {
  ...
  public boolean checkNodeForCorrectCalculationSyntactics(int vertexIndex) {
    return ((Vertex) graph.getVertexes().get(vertexIndex)).checkForCorrectSyntax();
  }
    ...
    while (restart) 
    {
      ...
      for (int a = 0; a < graph.getVertexes().size(); a++) 
      {
        ...
        if (v.getGraphsButtonStatus() == v.NOSTATUS) 
        {
          ...
          else if (v.getType()==Vertex.STOCK) 
          {
            for (int i = 0; i < v.outedges.size(); i++) 
            {
              Vertex current = (Vertex)v.outedges.get(i).end;
