...
public class InputsPanel extends javax.swing.JPanel implements ItemListener {
  ...
  public void initValues() 
  {
    ...
    else if (currentVertex.getInputsButtonStatus() == currentVertex.WRONG) 
    {
      ...
      if (((currentVertex.getType()==amt.graph.Vertex.FLOW) || (currentVertex.getType()==amt.graph.Vertex.STOCK)) && currentVertex.getInputsSelected()) 
      {
        radioPanel.setBackground(Selectable.COLOR_CORRECT);
        valueButton.setBackground(Selectable.COLOR_CORRECT);
        inputsButton.setBackground(Selectable.COLOR_CORRECT);
  ...
  public void initInitialState() 
  {
    ...
    else if ((currentVertex.getType()==amt.graph.Vertex.FLOW) || (currentVertex.getType()==amt.graph.Vertex.STOCK) || currentVertex.getInputsSelected()) 
    {
      inputsButton.setSelected(true);
      inputsButtonPreviouslySelected =true;
      currentInputPanel.setVisible(true);
    } 
    else if ((currentVertex.getType()==amt.graph.Vertex.NOTYPE) && inputsButton.isSelected())
      currentInputPanel.setVisible(true);
    ...
    if (!currentVertex.inedges.isEmpty()) 
    {
      ...
      for (int i = 0; i < currentVertex.inedges.size(); i++) 
      {
        for (int j = 0; j < boxList.size(); j++) 
        {
          ...
          if (boxList.get(j).getText().equalsIgnoreCase(currentVertex.inedges.get(i).start.getNodeName()) && (currentVertex.getType()!=amt.graph.Vertex.STOCK) && !currentVertex.inedges.get(i).edgetype.equalsIgnoreCase("flowlink")) {
            boxList.get(j).setSelected(true);
            
          }
          else if (boxList.get(j).getText().equalsIgnoreCase(currentVertex.inedges.get(i).start.getNodeName()) && (currentVertex.getType()==amt.graph.Vertex.STOCK)){
            boxList.get(j).setSelected(true);
      ...
      if ((currentVertex.getType()==amt.graph.Vertex.STOCK) && !currentVertex.outedges.isEmpty()) 
    ...
    while (restart) 
    {
      ...
      for (int a = 0; a < g.getVertexes().size(); a++) 
      {
        ...
        if (v.getGraphsButtonStatus() == v.NOSTATUS) 
        {
          ...
          if ((v.getType()==amt.graph.Vertex.CONSTANT) || (v.getType()==amt.graph.Vertex.FLOW)) 
          {
          ...
          else if (v.getType()==amt.graph.Vertex.STOCK) 
          {
            for (int i = 0; i < v.outedges.size(); i++) 
            {
              Vertex current = (Vertex)v.outedges.get(i).end;
  ...
  public void itemStateChanged(ItemEvent e) {
    ...
    if (initializing == false) 
    {
      ...
      if (!skip || (skip && (currentVertex.getType()==amt.graph.Vertex.FLOW))) 
      {
  ...
  @SuppressWarnings("unchecked")
    ...
    private void valueButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_valueButtonActionPerformed
      ...
      if(this.currentVertex.getType()==amt.graph.Vertex.CONSTANT) //the selecion is right
      ...
      if (currentVertex.getType()==amt.graph.Vertex.NOTYPE) {
        currentVertex.setType(amt.graph.Vertex.CONSTANT);
      }
      parent.getCalculationsPanel().update();
      parent.getCalculationsPanel().initButtonOnTask();
    ...
    private void inputsButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_inputsButtonActionPerformed
      ...
      if((currentVertex.getType()==amt.graph.Vertex.FLOW) || (currentVertex.getType()==amt.graph.Vertex.STOCK)) 
      {
        //change the input and graph status so that the (i), and (g) (possibly the (c)) circles
        //on the vertex turns white
        currentVertex.setInputsButtonStatus(currentVertex.NOSTATUS);
        ...
        if (!this.giveUpPressed && !currentVertex.getIsCalculationTypeCorrect())
           currentVertex.setType(amt.graph.Vertex.NOTYPE);
        ...
        if(this.currentVertex.getType() == amt.graph.Vertex.CONSTANT) { //the selection is correct
          currentVertex.setType(amt.graph.Vertex.AUXILIARY);
          resetColors();
...
private boolean correctnessOfInputs() {
  ...
  if (correctVertex == null)
    ...
    if((correctVertex.getType()==amt.graph.Vertex.FLOW) || (correctVertex.getType()==amt.graph.Vertex.AUXILIARY)) 
    {
    ...
    else if(correctVertex.getType()==amt.graph.Vertex.STOCK) 
    {
...
public boolean checkForCorrectInputs() 
{
  ...
  else 
  {
    ...
    {
      if ((currentVertex.getType()==amt.graph.Vertex.CONSTANT)!=valueButton.isSelected()) 
      {
        correct = false;
        correctType = false;
        correctInput = false;
      ...
      if (!initializing) 
      {
        ...
        if (!parent.getDescriptionPanel().duplicatedNode(currentVertex)) 
        {
          ...
          else 
          {
            ...
            if ((correctVertex.getType()==amt.graph.Vertex.CONSTANT) && valueButton.isSelected())
            {
              correct = true;
              correctType = true;
              correctInput = true;
            }
            else if ((correctVertex.getType()==amt.graph.Vertex.CONSTANT) && inputsButton.isSelected()){
              correct = false;
              correctType = false;
              correctInput = false;
            }
            else if (((correctVertex.getType()==amt.graph.Vertex.FLOW) || (correctVertex.getType()==amt.graph.Vertex.STOCK)) && valueButton.isSelected()){
              correct = false;
              correctType = false;
              correctInput = false;
            }
          ...
          if (areAllCorrectInputsAvailable() != false) 
          {
            ...
            if(correctinput)
            {
              ...
              for (JCheckBox box : boxList) 
              {
                ...
                else if ((correctVertex.getType()==amt.graph.Vertex.STOCK) && correctVertex.getCorrectOutputs().contains("flowlink - " + box.getText().replaceAll("_", " ")))
                  box.setSelected(true);
                else if (correctVertex.getCorrectInputs().contains("flowlink - " + box.getText().replaceAll("_", " ")) && (correctVertex.getType()==amt.graph.Vertex.STOCK))
                  box.setSelected(true);
              }
