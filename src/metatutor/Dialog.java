...
public class Dialog {
  ...
  public static void genChooseStartingForDebug(List<String> candidates){
    if(!GlobalVar.silentMT){
    String content="Since that more than one nodes contain red indicators, you need to choose one of them to get started. "
            + "It doesn't matter which one you select, because eventually you need to fix all of them.";
    String []params=new String[candidates.size()+1];
    params[0]="-";
    for(int i=0;i<candidates.size();i++)
